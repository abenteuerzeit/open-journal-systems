...
(function (domGlobals) {
    ...
    var URI = function (url, settings) {
      ...
      if (url.indexOf('/') === 0 && !isProtocolRelative) {
        url = (baseUri ? baseUri.protocol || 'https' : 'https') + '://mce_host' + url;
      }
      if (!/^[\w\-]*:?\/\//.test(url)) {
        ...
        if (settings.base_uri.protocol == '') {
          ...
          url = /([^#?]*)([#?]?.*)/.exec(url);
          url = (baseUri && baseUri.protocol || 'https') + '://mce_host' + self.toAbsPath(baseUrl, url[1]) + url[2];
        }
      }
    ...
    URI.getDocumentBaseUrl = function (loc) {
      var baseUrl;
      if (loc.protocol.indexOf('https') !== 0 && loc.protocol !== 'file:') {
        baseUrl = loc.href;
      } else {
